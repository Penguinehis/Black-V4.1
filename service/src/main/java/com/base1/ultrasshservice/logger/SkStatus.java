	...
	*/
    ...
    public synchronized static void updateStateString(String state, String msg, int resid, ConnectionStatus level, Intent intent) {
        ...
        if (mLastLevel == ConnectionStatus.LEVEL_CONNECTED &&
            ...
            newLogItem(new LogItem((LogLevel.DEBUG), String.format("Ignoring BSE1VPN Status in CONNECTED state (%s->%s): %s", state, level.toString(), msg)));
            return;
        }
        ...
        //newLogItem(new LogItem((LogLevel.DEBUG), String.format("BSE1VPN Novo Status (%s->%s): %s",state,level.toString(),msg)));
    }
